...
import static Clases.Usuario.separarNombres;
import static Clases.Usuario.stringPropio;
    ...
    public static void consultarRUC(String ruc, Empresa empresa) {
        try {
            ...
            if (responseBody.get("success").asBoolean()) {
                ...
                empresa.setNombre(data.get("nombre_o_razon_social").asText()); // No formatear razón social
    ...
    public static void consultarDni(String dni, Usuario usuario) {
        ...
        try {
            ...
            if (responseBody.get("success").asBoolean()) {
                ...
                // Formatear y asignar nombres y apellidos
                VarTemp = data.get("nombres").asText();
                usuario.setNombres(stringPropio(VarTemp));
                
                Nombre = separarNombres(VarTemp);
                usuario.setFirstName(stringPropio(Nombre[0]));
                usuario.setSecondName(stringPropio(Nombre[1]));
                usuario.setFirstLastName(stringPropio(data.get("apellido_paterno").asText()));
                usuario.setSecondLastName(stringPropio(data.get("apellido_materno").asText()));
